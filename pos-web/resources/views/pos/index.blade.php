    ...
    <div class="pos-products">
        ...
        <div class="prod-grid" id="prodGrid">
            ...
            <div class="prod-card {{ $product->stock <= 0 ? 'sold-out' : '' }}"
                ...
                @if($product->stock <= 0)
                    <div class="sold-out-badge"><span class="sold-out-text">HABIS</span>
            </div>
            @endif
        </div>
        @endforeach
...
<div class="pos-cart">
    <div class="cart-hd">
        <span class="cart-hd-title">
            <svg style="width:16px;height:16px;stroke:#fff;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round" viewBox="0 0 24 24">
...
<script>
    ...
    function changeQty(id, d) {
        ...
        if (cart[id].qty <= 0) {
            ...
            showToast('Stok maks tercapai!', 'error');
        }
        renderCart();
    }
    ...
    function buildInvoicePreview(d) {
        const now = new Date();
        const dateWIB = now.toLocaleString('id-ID', {
...
<br><br>
        ...
        if (!win) {
            showToast('Popup diblokir. Izinkan popup di browser Anda.', 'error');
            return;
        }
